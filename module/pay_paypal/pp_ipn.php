...
$order_info = sql_qquery("SELECT * FROM ".$db_prefix."order_summary WHERE order_id='$order_id' LIMIT 1");
$total = $order_info['order_total'] + $order_info['order_payment_fee'] + $order_info['order_shipping_fee'] + $order_info['order_tax'] - $order_info['gift_discount'] - $order_info['store_discount'];
if (!$order_info) {
...
if ($mypayment_status == 'P') {
    ...
    sql_query("UPDATE ".$db_prefix."order_summary SET order_paystat='P' WHERE order_id='$order_id' LIMIT 1");
