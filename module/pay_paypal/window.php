...
switch ($payment_cmd) {
    case 'form':
        ...
        $hidden_val = array('rm' => 2,
                             ...
                             'amount' => number_format(($summary['order_total'] + $summary['order_payment_fee']) * $cv, 2),
                             'discount_amount' => number_format($summary['gift_discount'] * $cv, 2),
                             'shipping' => number_format($summary['order_shipping_fee'] * $cv, 2),
                             'tax' => number_format($summary['order_tax'] * $cv, 2),
                             'quantity' => '1',
                             'return' => $config['site_url'],
                             'cbt' => 'Click here to return to '.$config['site_name'],
                             'first_name' => $bill_fn,
                             'last_name' => $bill_ln,
                             'address1' => $user['bill_address'],
                             'address2' => $user['bill_address2'],
                             'city' => $user['bill_city'],
                             'state' => $user['bill_state'],
                             'zip' => $user['bill_zip'],
                             'country' => $user['bill_country'],
                             'day_phone_a' => $user['phone'],
