...
<form method="get" id="search_filter_form">
	...
	<p><b>{$l_price_range}</b><br />
	<input type="text" id="price_select" name="price" value="" data-type="double" data-prefix="{$num_currency}" data-min="0" data-max="{$price_max}" data-from="{$price_from}" data-to="{$price_to}" /></p>

	<p><b>{$l_distro}</b><br />
	{$distro_select}</p>
	{$cf_list}
	<button type="button" onclick="search_filter_submit()" class="btn btn-default"><span class="glyphicon glyphicon-filter"></span> {$l_submit}</button>
</form>
<script type="text/javascript">
	...
	$("#price_select").ionRangeSlider({onFinish:function () { search_filter_submit()}});
</script>
<!-- ENDSECTION -->