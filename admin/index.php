...
// display current trx
$txt['block_trx_item'] = ''; $i = 0;
$res = sql_query("SELECT * FROM ".$db_prefix."order_summary ORDER BY order_date DESC, order_id DESC LIMIT 5");
while ($row = sql_fetch_array($res)) {
    $i++;
    if (substr($row['user_id'], 0, 6) == 'guest*') {
        $row['user_id'] = '[ Xpress Checkout ]';
    }
    $row['n'] = $i;
...
$ss5 = sql_qquery("SELECT COALESCE(SUM(order_status='E'), 0) AS num_e, COALESCE(SUM(order_status='P'), 0) AS num_p,
               COALESCE(SUM(order_status='S'), 0) AS num_s, COALESCE(SUM(order_status='D'), 0) AS num_d, COALESCE(SUM(order_status='C'), 0) AS num_c, COALESCE(SUM(order_status='X'), 0) AS num_x,
               COALESCE(SUM(order_total), 0) AS num_tt FROM ".$db_prefix."order_summary");
$ss5['num_tt'] = num_format($ss5['num_tt'], 0, 1);

$since7 = convert_date($sql_now, 'sql', -7);
$ss3 = sql_qquery("SELECT COUNT(*) AS num_m, SUM(user_since > '$since7') AS num_mn FROM ".$db_prefix."user");
...
// ss1 = login info, ss2 = new users, ss3 = db info, ss4 = comments, ss5 = trx
$txt = array_merge($txt, $ss3, $ss4, $ss5);
