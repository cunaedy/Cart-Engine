...
switch ($cmd) {
    case 'save':
        ...
        if (!empty($display_name)) {
            sql_query("UPDATE ".$db_prefix."module SET mod_name='$display_name' WHERE mod_id='pay_cod' LIMIT 1");
        }
        admin_die('admin_ok');
    break;
    ...
    default:
        ...
        $row = sql_qquery("SELECT * FROM ".$db_prefix."module WHERE mod_id='pay_cod' LIMIT 1");
        
        // init some stuffs
        $row['config_title'] = 'Payment Module: Cash on Delivery';
        $row['mod_id'] = 'pay_cod';
